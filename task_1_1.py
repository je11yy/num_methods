...
def get_triangular_matrix(matrix_a, matrix_b, identity_matrix):
    ...
    for column in range(length):
        ...
        determinant *= matrix_a[column][column]
...
matrix = np.array(coefficients_a)

identity_matrix = get_identity_matrix(len(coefficients_a))
determinant = get_triangular_matrix(coefficients_a, coefficients_b, identity_matrix)
Gauss(coefficients_a, coefficients_b)
print()

print("determinant: {0:.3f}".format(determinant))
print("real determinant: {0:.3f}".format(np.linalg.det(matrix) ))
print()
...
print()
print("real reverse matrix:")
print_matrix(np.linalg.inv(matrix))

print()
print("mult matrix on reverse matrix:")
result = np.matmul(matrix, np.array(identity_matrix))
print_matrix(result)